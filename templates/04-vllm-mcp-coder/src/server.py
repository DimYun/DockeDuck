...
from .hardware import context_for, detect_gpu, recommend_model as _recommend_model
...
@mcp.tool()
async def recommend_model(prefer: str = "quality") -> str:
    """Detect this machine's GPU and recommend the best vLLM model to run here.

    Numbers come from the DockeDuck 6 GB benchmark (experiments/RESULTS.md) plus exact
    KV-cache math, so the pick and its context window are sized to the detected VRAM.

    prefer — 'quality' (default, highest test pass rate) · 'context' (largest window) ·
             'speed' (smallest/fastest model).

    Returns the recommended model, the context window that fits, and the exact .env change
    to apply. vLLM loads the model at startup, so applying a change means editing
    templates/04-vllm-mcp-coder/.env and restarting (`make start` / `make start-bench`).
    """
    rec = _recommend_model(prefer=prefer)
    if not rec.get("name"):
        return (f"# Hardware: {rec['gpu']} ({rec['vram_gb']} GB)\n\n{rec['reason']}")
    thinking = "  (set ENABLE_THINKING=true for +quality)" if rec["thinking"] else ""
    return (
        f"# Recommended model for {rec['gpu']} ({rec['vram_gb']} GB VRAM)\n\n"
        f"VLM_MODEL={rec['name']}\n"
        f"VLM_MAX_MODEL_LEN={rec['recommended_context']}\n\n"
        f"Expected quality : {rec['quality']}% local · {rec['rescue']} tasks pass with one rescue{thinking}\n"
        f"Why              : {rec['note']}\n"
        f"How to apply     : set the two lines above in templates/04-vllm-mcp-coder/.env, "
        f"then restart (make start / make start-bench)."
    )
...
@mcp.tool()
async def recommend_context_window(model: str = "") -> str:
    """Recommend the largest context window (VLM_MAX_MODEL_LEN) that fits the detected GPU
    for a given model (defaults to the currently configured VLM_MODEL).

    Context is bounded by VRAM and the model's GQA KV-head count — a bigger model can have a
    *smaller* usable window. This returns the computed maximum and the .env line to set it.
    """
    from .hardware import MODELS
    hw = detect_gpu()
    # Default to the configured model, or (if unset) the best model for this GPU.
    target = model.strip() or os.getenv("VLM_MODEL", "") or (_recommend_model().get("name") or "")
    spec = next((m for m in MODELS if m["name"] == target), None)
    if not hw["gpu"]:
        return (f"# Hardware: {hw['name']}\n\nNo CUDA GPU detected — vLLM needs a GPU. "
                f"Use the Ollama template (05) for CPU inference.")
    if spec is None:
        return (f"# Hardware: {hw['name']} ({hw['vram_gb']} GB)\n\n"
                f"Unknown model '{target}'. Known models: "
                + ", ".join(m['name'] for m in MODELS)
                + ".\nUse recommend_model to pick one, or pass model=<HF name>.")
    ctx = context_for(spec, hw["vram_gb"])
    if ctx < 2048:
        return (f"# {target} does not fit {hw['vram_gb']} GB with a usable context. "
                f"Pick a smaller model with recommend_model(prefer='speed').")
    return (
        f"# Context window for {target} on {hw['name']} ({hw['vram_gb']} GB)\n\n"
        f"VLM_MAX_MODEL_LEN={ctx}\n\n"
        f"Basis   : {spec['kv_heads']} KV heads × {spec['layers']} layers "
        f"(~{2 * spec['layers'] * spec['kv_heads'] * spec['head_dim'] * 2 // 1024} KB/token), "
        f"capped at the model's {spec['rope_max']} RoPE limit.\n"
        f"Apply   : set VLM_MAX_MODEL_LEN in templates/04-vllm-mcp-coder/.env, then restart. "
        f"Keep MAX_TOKENS ≤ half this value so the fix loop's history never overflows."
    )
